...
describe('AgentWorkspace', () => {
  it('renders the responsive shared shell without a Chat/Tool switcher', () => {
    const markup = renderToStaticMarkup(
      <AgentWorkspace
        mode="chat"
        capabilities={{ chatAllowed: true, chatConfigured: true, chatUsable: true, tool: true, openShopTool: false, defaultMode: 'chat' }}
    ...
    expect(markup).not.toContain('aria-controls="agent-chat-panel"')
  ...
  it('renders the embedded composer inside the central column only while active', () => {
    const props = {
      mode: 'chat' as const,
      capabilities: { chatAllowed: true, chatConfigured: true, chatUsable: true, tool: true, openShopTool: false, defaultMode: 'chat' as const },
      activeTaskByMode: { chat: 'task-old', tool: 'task-new' },
      onActiveTaskChange: vi.fn(),
  ...
  it('single-capability mode hides the switcher and does not expose unavailable mode', () => {
    const markup = renderToStaticMarkup(
      <AgentWorkspace
        mode="tool"
        capabilities={{ chatAllowed: true, chatConfigured: false, chatUsable: false, tool: true, openShopTool: false, defaultMode: 'tool' }}
